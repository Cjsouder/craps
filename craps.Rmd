...
check_cheating <- tibble(IB = map_int(1:n, ~ roll_two_dice()),
                         JM = map_int(1:n, ~ roll_two_dice())) %>%
  mutate(IB_craps = IB %in% c(7,11)) %>%
  mutate(JM_craps = JM %in% c(7,11)) %>%
  mutate(winner = "Tie") %>%
  mutate(winner = ifelse(IB_craps & !JM_craps, "IB", winner)) %>%
  mutate(winner = ifelse(!IB_craps & JM_craps, "JM", winner)) %>%
